...
bucket_name = 'datasetsads'
object_key = 'testcase.csv'
...
class Preprocess(luigi.Task):
    ...
    def run(self):
        ...
        df_clean = pd.DataFrame(
            columns=['customer_ID', 'num_quotes', 'state', 'location', 'group_size', 'homeowner', 'car_age',
    ...
    def output(self):
        return luigi.LocalTarget("process_test2.csv")


class custMatch(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget("predictCat_df.csv")


class catPrediction(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget("df_predicted_cat.csv")
...
class join(luigi.Task):
    ...
    def output(self):
        return luigi.LocalTarget("submission.csv")


if __name__ == '__main__':
    luigi.build([Preprocess(), custMatch(), catPrediction(), join()], local_scheduler=True)
